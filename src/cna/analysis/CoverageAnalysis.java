...
import cna.config.Config;
import cna.data.Alignment;
import cna.data.Annotation;
import cna.data.Gene;
import cna.data.Genome;
import cna.data.Region;
import cna.data.Repeat;
import cna.data.RepeatMasker;
import cna.parallel.ParallelArrayAdder;
import cna.util.HistogramBuilder;
import cna.util.RegionImporter;
import cna.util.SVGBuilder;
...
public class CoverageAnalysis {
	...
	public static void evaluateGenomeCoverage(ArrayList<Alignment> alignments, double threshold, int minSize) {
		...
		for (String chromosome : alignments.get(0).getGenome().getChromosomeNames())
			if (!(chromosome.equals("Y") || chromosome.equals("MT")))
	...
	public static void evaluateSortedRegions(ArrayList<Alignment> alignments, double threshold, int minSize,
		...
		SVGBuilder.saveSVG(FragmentSizeAnalysis.analyzeFragmentSize(L1TaRegions, alignments),
				FragmentSizeAnalysis.analyzeFragmentSize(sortedRegions.get("L1"), alignments),
				FragmentSizeAnalysis.analyzeFragmentSize(sortedRegions.get("centr"), alignments),
				FragmentSizeAnalysis.analyzeFragmentSize(sortedRegions.get("Simple_repeat"), alignments),
				"FragmentSizesSortedStrong");
	}
	...
	private static ArrayList<Region> getCoveredRegions(ArrayList<Alignment> alignments, double threshold, int minSize,
		...
		for (String chromosome : genome.getChromosomeNames()) {
			if (chromosome.equals("Y") || chromosome.equals("MT")) // exclude Y chromosome and mitochondrium
				continue;
			System.out.println("Chromosome: " + chromosome);
	 ...
	 * @param alignments list of alignments
	 */
	public static void plotAll(ArrayList<Region> regions, ArrayList<Alignment> alignments) {
